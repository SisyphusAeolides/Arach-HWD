...
idris_source="$root/formal/idris2/HardwarePlan.idr"
agda_source="$root/formal/agda/HardwarePlan.agda"
...
scratch="$(mktemp -d "${TMPDIR:-/tmp}/arach-hwd-formal.XXXXXXXX")"
trap 'rm -rf -- "$scratch"' EXIT
cp "$idris_source" "$agda_source" "$scratch/"
(
    cd "$scratch"
    "$idris" --check HardwarePlan.idr
    XDG_DATA_HOME="$scratch/data" XDG_CONFIG_HOME="$scratch/config" \
        "$agda" --no-libraries --safe --without-K HardwarePlan.agda
)
